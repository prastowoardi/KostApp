...
export default function AdminComplaints() {
    ...
    const handleUpdateStatus = async (status) => {
        if (!reply && status === 'resolved') {
            showAlert("Oops", "Berikan pesan solusi sebelum menyelesaikan laporan.", "warning");
            return;
        }
        ...
        try {
            ...
            showAlert("Gagal", "Terjadi kesalahan saat memperbarui laporan.", "error");
        }
    };

    const filteredComplaints = complaints.filter(item => 
        filterStatus === 'all' ? true : 
        (filterStatus === 'process' ? item.status === 'in_progress' : item.status === filterStatus)
    );
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity 
            style={[
                styles.card, 
                { borderLeftColor: item.status === 'open' ? '#e74a3b' : item.status === 'in_progress' ? '#f6c23e' : '#1cc88a' }
            ]}
            onPress={() => {
        ...
        >
            <View style={styles.cardHeader}>
                ...
                <View style={[
                    styles.statusBadge, 
                    { backgroundColor: item.status === 'open' ? '#ffeaea' : item.status === 'in_progress' ? '#fff4e5' : '#eafaf1' }
                ]}>
                    <Text style={[
                        styles.statusText, 
                        { color: item.status === 'open' ? '#e74a3b' : item.status === 'in_progress' ? '#d39e00' : '#1cc88a' }
                    ]}>
                        {item.status === 'in_progress' ? 'PROSES' : item.status.toUpperCase()}
                    </Text>
                </View>
            </View>
        ...
        <View style={styles.container}>
                ...
                {['all', 'open', 'process', 'resolved'].map((status) => (
                    ...
                    >
                        <Text style={[styles.filterText, filterStatus === status && styles.filterTextActive]}>
                            {status === 'all' ? 'All' : status === 'open' ? 'PENDING' : status.toUpperCase()}
                        </Text>
                    </TouchableOpacity>
                ))}
            </View>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    refreshControl={
                        <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#b21f1f']} />
                    }
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <TouchableOpacity style={styles.closeBtn} onPress={() => setModalVisible(false)}>
                                <Text style={{ color: '#858796', fontWeight: 'bold' }}>Tutup</Text>
                            </TouchableOpacity>
                        </ScrollView>
                    </View>
                </View>
...
const styles = StyleSheet.create({
    ...
    header: { paddingTop: 60, paddingBottom: 50, paddingHorizontal: 20, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 },
    headerTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
    headerTitle: { fontSize: 20, fontWeight: 'bold', color: 'white' },
    backBtn: { backgroundColor: 'rgba(255,255,255,0.2)', padding: 8, borderRadius: 12 },
    statsRow: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, marginTop: -35, marginBottom: 10 },
    statsCard: { flex: 0.31, backgroundColor: 'white', borderRadius: 15, padding: 15, alignItems: 'center', elevation: 4, borderBottomWidth: 4 },
    statsNumber: { fontSize: 18, fontWeight: 'bold', color: '#2c3e50' },
    statsLabel: { fontSize: 10, color: '#858796', marginTop: 2, fontWeight: '600' },
    filterContainer: { flexDirection: 'row', justifyContent: 'space-around', paddingVertical: 15, backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#eee' },
    filterBtn: { paddingVertical: 6, paddingHorizontal: 15, borderRadius: 20, backgroundColor: '#f1f3f9' },
    ...
    modalContent: { backgroundColor: 'white', borderTopLeftRadius: 30, borderTopRightRadius: 30, padding: 25, maxHeight: SCREEN_HEIGHT * 0.85 },
    modalTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 15, textAlign: 'center' },
    modalLabel: { fontSize: 11, fontWeight: 'bold', color: '#34495e', marginBottom: 6, textTransform: 'uppercase' },
    bubbleTenant: { backgroundColor: '#f8f9fc', padding: 12, borderRadius: 12, marginBottom: 15, borderStyle: 'dashed', borderWidth: 1, borderColor: '#d1d3e2' },
    modalDesc: { fontSize: 14, color: '#2c3e50', lineHeight: 20 },
    input: { backgroundColor: '#f1f3f9', borderRadius: 12, padding: 15, textAlignVertical: 'top', marginBottom: 20, height: 100, borderWidth: 1, borderColor: '#d1d3e2' },
    modalActions: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 },
    modalBtn: { flex: 0.48, padding: 15, borderRadius: 12, alignItems: 'center', elevation: 2, flexDirection: 'row', justifyContent: 'center' },
